...
sepvalue_=1/60 #for testing to check if it affects the problem
####################################################################################
...
np.random.seed(key)
####################################################################################
print("Solve the problem incrementally (i.e., provide the preference at each iteration) for alternative pref fun")
# solve same problem, but incrementally
...
xopt4 = xbest4
fbest_seq4 = list(map(fun, X4[prob4.ibest_seq]))
