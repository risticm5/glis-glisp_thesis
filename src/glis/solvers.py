...
def facquisition(xs, Xs, F, X_all, F_all, useRBF, rbf_xs, W, delta_E, dF,
    ...
    if ii[0].size > 0:
        ...
        if W is None:  # when all the current samples are infeasible
            ...
            if useRBF:
                v = rbf_xs
                fhat = (np.sum(v * W)).item() # W - RBF weights
            else:
                fhat = (np.sum(F * w) / sw).item() # w - IDW function weights
        ...
        if not scale_delta:
            ...
            if W is not None:  # when there exists feasible samples
                dhat = dhat * 2. / np.pi * dF + alpha * np.sqrt(np.sum(w * (F - fhat) ** 2) / sw) # EXPLORATION part of acquisition function!!!
        else:
...
class GLIS_base(object):
    ...
    def initialize_(self):
        ...
        """
        # lhs(self.nvars, self.n_initial_random, "m") generates n_initial_random samples in a space of dimension nvars
        # The transformation 2. * lhs(...) - 1. scales and shifts the samples to lie within the range [−1,1]

        if not self.feasible_sampling:
            Xs = 2. * lhs(self.nvars, self.n_initial_random, "m") - 1.
            if (not self.isLinConstrained and not self.isNLConstrained): #if there is no constraints, all points are feasible
                self.KnownFeasible = list(np.ones(self.n_initial_random))
            else:
                for i in range(self.n_initial_random): #if there are constraints, check for feasibility
                    self.KnownFeasible.append(self.isKnownFeasible(Xs[i]))
        else: #function generates samples while ensuring they are feasible
            ...
            self.KnownFeasible = list(np.ones(self.n_initial_random))
        self.X = list(Xs * self.dd + self.d0) #rescaling
        return Xs
    ...
    def get_delta_adpt(self, Xs, constraint_set, delta_const_default):
        ...
        """
        #The function seems to be calculating an adaptive exploration parameter (delta_adpt) based on how well the constraints are satisfied
        #more exploration when the constraints are not well satisfied and less exploration when they are well satisfied.
...
class GLISp(GLIS_base):
    ...
    def initialize(self):
        ...
        self.expected_max_evals=25 # this must be added in order to have same execution with and without solve() functiion!

        if self.sepvalue is None:
            self.sepvalue = 1. / self.expected_max_evals
        
        #self.expected_max_evals is differenet if we run solve or not! So it will determine when is done recalibration of RBF(epsilon is updated)
